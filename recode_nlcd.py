...
def recode_nlcd(indir, outdir):

    driver = gdal.GetDriverByName("GTiff")
    
    for mainroot, folders, files in os.walk(indir):
        ...
        for folder in folders:
            ...
            in_nlcd_list = glob.glob(infolder + os.sep + "*.img")
            
            if len(in_nlcd_list) == 0:

                in_nlcd_list = glob.glob(infolder + os.sep + "*.tif")
            ...
            for idx, nlcd in enumerate(in_nlcd_list):
                ...
                outfolder = outdir + os.sep + folder
                
                if not os.path.exists(outfolder):
                    
                    os.makedirs(outfolder)
                
                outname = outfolder + os.sep + fname + "_rc.tif"
